...
import { danisanlariGetir, kayitFormunuBaslat } from './modules/danisan.js?v=final3';
import { randevulariGetir, randevuFormunuBaslat } from './modules/randevu.js?v=final3';
...
// ================= GECİKMELİ, GARANTİLİ PDF MOTORU =================
const pdfOlusturVeIndir = (htmlIcerik, dosyaAdi) => {
    ...
    // 150 milisaniye bekle (Tarayıcı DOM'u çizsin), sonra fotoğraf çek.
    setTimeout(() => {
        html2pdf().set(opt).from(tempDiv).save().then(() => {
            ...
            window.showToast('PDF Hatası oluştu.', 'error'); 
            tempDiv.remove();
        });
    }, 150);
}
    ...
    const htmlRapor = `
        <div style="padding: 40px 50px; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #1e293b; background: white; width: 800px; box-sizing: border-box;">
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 30px; font-size: 13px;">
                ...
                <tr>
                    <td style="border: 1px solid #e2e8f0; padding: 10px; font-weight: bold; background: #f8fafc;">Su Tüketimi / Aktivite</td>
...
if(frmOlcum) {
    frmOlcum.onsubmit = async (e) => {
        ...
        if(!error) { 
            ...
            window.showToast('Ölçüm başarıyla kaydedildi!', 'success'); 
            window.olcumleriGetir(window.aktifHastaId); 
        }
    };
}

// ================= YENİ TAHLİL KAYDETME MOTORU =================
...
if(frmTahlil) {
    frmTahlil.onsubmit = async (e) => {
        ...
        if(!error) { 
            ...
            window.showToast('Kan tahlili kaydedildi!', 'success'); 
            window.tahlilleriGetir(window.aktifHastaId); 
        }
    };
}

// ================= KASA / FİNANS KAYDETME MOTORLARI =================
...
if(frmCariHizmet) {
    frmCariHizmet.onsubmit = async (e) => {
        ...
        if(!error) { 
            ...
            window.showToast('Hizmet eklendi!', 'success'); 
...
if(frmCariOdeme) {
    frmCariOdeme.onsubmit = async (e) => {
        ...
        if(!error) { 
            ...
            window.showToast('Ödeme alındı!', 'success'); 
            window.cariHareketleriGetir(window.aktifHastaId); 
            window.finanslariGetir(); 
        }
    };
...
if(frmGenelFinans) {
    frmGenelFinans.onsubmit = async (e) => {
        ...
        if(!error) { 
            ...
            window.showToast('Tahsilat kaydedildi!', 'success'); 
            window.finanslariGetir(); 
        }
    };
}

// ================= DİYET VE ŞABLON ÇEKME İŞLEMLERİ =================
