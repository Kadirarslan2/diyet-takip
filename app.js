...
import { danisanlariGetir, kayitFormunuBaslat } from './modules/danisan.js?v=raporpro1';
import { randevulariGetir, randevuFormunuBaslat } from './modules/randevu.js?v=raporpro1';
...
// ================= KLİNİK RAPORU (ÖLÇÜM VE TAHLİLLER EKLENDİ!) =================
window.pdfIndir = async function() {
    ...
    const htmlRapor = `
        <div style="padding: 30px 40px; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #1e293b; background: white; width: 800px; box-sizing: border-box;">
            ...
            <div style="border: 1px solid #e2e8f0; background: #f8fafc; padding: 12px; font-size: 11px; line-height: 1.5; min-height: 60px; border-radius: 4px;">
    ...
    tempDiv.style.position = 'absolute'; tempDiv.style.top = '0'; tempDiv.style.left = '-9999px'; 
    document.body.appendChild(tempDiv);

    const opt = { margin: 10, filename: `${d.ad}_${d.soyad}_Klinik_Raporu.pdf`, image: { type: 'jpeg', quality: 1 }, html2canvas: { scale: 2, useCORS: true }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' } };
...
window.diyetPdfIndir = async function(diyetId) {
    ...
    const opt = { margin: 10, filename: `Diyet_${adSoyad}.pdf`, image: { type: 'jpeg', quality: 1 }, html2canvas: { scale: 2, useCORS: true }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' } };
    html2pdf().set(opt).from(tempDiv).save().then(() => { window.showToast('Diyet Listesi İndirildi!', 'success'); tempDiv.remove(); });
}
...
if(frmSablon) { 
    frmSablon.onsubmit = async (e) => { 
        ...
        const { error } = await supabase.from('sablonlar').insert([{ 
            baslik: v('s-baslik'), sabah: v('s-sabah'), ara1: v('s-ara1'), ogle: v('s-ogle'), ara2: v('s-ara2'), aksam: v('s-aksam'), ara3: v('s-ara3'), icerik: v('s-notlar') 
        }]); 
        if(!error) { 
            frmSablon.reset(); window.closeModal('modal-sablon'); 
            window.showToast('Şablon kaydedildi!'); 
            window.sablonlariGetir(); 
        } else {
            window.showToast('HATA: Şablon veritabanına kaydedilemedi!', 'error');
            console.error("Şablon Kayıt Hatası:", error);
        }
    }; 
}

// ================= ŞABLON ÇEKME İŞLEMLERİ =================
window.sablonlariGetir = async function() { 
    const list = document.getElementById('sablon-listesi'); const sel = document.getElementById('diy-sablon-secici'); 
    try {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.randevulariGetir(); window.sablonlariGetir(); window.finanslariGetir();
});