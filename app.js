...
import { danisanlariGetir, kayitFormunuBaslat } from './modules/danisan.js?v=kadirManuelCozum';
import { randevuFormunuBaslat } from './modules/randevu.js?v=kadirManuelCozum';
...
window.showToast = function(mesaj, tip = 'success') {
    ...
    toast.innerHTML = `<i class="fas ${tip === 'success' ? 'fa-check-circle' : 'fa-exclamation-circle'}"></i> <span>${mesaj}</span>`;
...
// ================= KADİR'İN MANUEL PDF MOTORU (EKRAN GÖRÜNTÜSÜ ALMAZ, %100 NET ÇIKAR) =================
const pdfOlusturVeIndir = (htmlIcerik, dosyaAdi) => {
    // 1. Ekranı kaplayan Manuel Önizleme Modalı
    ...
    // 2. Üstteki Kontrol Barı
    const topBar = document.createElement('div');
    topBar.className = "print-gizle"; // PDF'in içine çıkmaması için
    ...
    topBar.innerHTML = `
        ...
        <div style="display:flex; gap:10px;">
            <button id="btn-kapat" style="background:#ef4444; color:white; border:none; padding:8px 16px; border-radius:6px; font-weight:bold; cursor:pointer;"><i class="fas fa-times mr-1"></i> İptal Et</button>
            <button id="btn-indir" style="background:#ffffff; color:#0f766e; border:none; padding:8px 16px; border-radius:6px; font-weight:bold; cursor:pointer;"><i class="fas fa-download mr-1"></i> PDF Olarak İndir</button>
        </div>
    `;

    // 3. A4 Kağıdı Görünümü (Senin gördüğün kısım)
    ...
    // Manuel İndirme İşlemi (Telefonun KENDİ orijinal PDF oluşturucusunu tetikler)
    document.getElementById('btn-indir').onclick = () => {
        // Arkadaki her şeyi gizleyip sadece A4 kağıdını sistemin yazıcısına gönderiyoruz
        ...
        document.title = dosyaAdi; // İnen dosyanın adı dilediğimiz gibi olsun
        
        window.print(); // BU KOD TELEFONDA DİREKT "PDF OLARAK KAYDET" EKRANINI AÇAR!
...
window.pdfIndir = async function() {
    ...
    if(olcumler && olcumler.length > 0) {
        olcumler.forEach(o => { olcumHtml += `<tr><td style="border: 1px solid #e2e8f0; padding: 8px;">${new Date(o.tarih).toLocaleDateString('tr-TR')}</td><td style="border: 1px solid #e2e8f0; padding: 8px; font-weight:bold; color:#0f766e;">${o.kilo} kg</td><td style="border: 1px solid #e2e8f0; padding: 8px;">%${o.yag||0} / %${o.kas||0}</td><td style="border: 1px solid #e2e8f0; padding: 8px;">${o.bel||'-'} / ${o.kalca||'-'}</td><td style="border: 1px solid #e2e8f0; padding: 8px;">${o.vki||'-'}</td></tr>`; });
    ...
    if(tahliller && tahliller.length > 0) {
        tahliller.forEach(t => { tahlilHtml += `<tr><td style="border: 1px solid #e2e8f0; padding: 8px;">${new Date(t.tarih).toLocaleDateString('tr-TR')}</td><td style="border: 1px solid #e2e8f0; padding: 8px; color:#b91c1c; font-weight:bold;">${t.b12||'-'} / ${t.d_vitamini||'-'}</td><td style="border: 1px solid #e2e8f0; padding: 8px;">${t.demir||'-'}</td><td style="border: 1px solid #e2e8f0; padding: 8px;">${t.kolesterol||'-'}</td><td style="border: 1px solid #e2e8f0; padding: 8px;">${t.aclik_sekeri||'-'}</td><td style="border: 1px solid #e2e8f0; padding: 8px;">${t.tsh||'-'}</td></tr>`; });
    ...
    const htmlRapor = `<div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #1e293b; width: 100%; box-sizing: border-box;"><div style="border-bottom: 3px solid #0f766e; padding-bottom: 15px; margin-bottom: 30px; display: flex; justify-content: space-between; align-items: flex-end;"><div><h1 style="color: #0f766e; margin: 0; font-size: 28px; font-weight: 900;">DİYETTAKİBİM KLİNİĞİ</h1><p style="margin: 5px 0 0 0; color: #64748b; font-size: 13px; font-weight: bold; text-transform: uppercase;">Kapsamlı Hasta Analiz Raporu</p></div><div style="text-align: right; color: #64748b; font-size: 12px;"><strong>Tarih:</strong> ${islemTarihi}<br><strong>Uzman:</strong> ${uzman}<br><strong>Protokol:</strong> ${d.protokol_no || '-'}</div></div><h3 style="background-color: #f8fafc; color: #334155; padding: 10px 15px; font-size: 14px; margin-bottom: 15px; border-left: 4px solid #0f766e; font-weight: bold;">Kişisel ve Tıbbi Bilgiler</h3><table style="width: 100%; margin-bottom: 30px; font-size: 13px; border-collapse: collapse;"><tr><td style="padding: 8px 0; border-bottom: 1px solid #f1f5f9; width: 50%;"><strong>Ad Soyad:</strong> ${d.ad} ${d.soyad}</td><td style="padding: 8px 0; border-bottom: 1px solid #f1f5f9; width: 50%;"><strong>Cinsiyet / Yaş:</strong> ${d.cinsiyet || '-'} / ${yas}</td></tr><tr><td style="padding: 8px 0; border-bottom: 1px solid #f1f5f9;"><strong>Kronik Hastalıklar:</strong> <span style="color:#b91c1c">${d.kronik_hastaliklar || '-'}</span></td><td style="padding: 8px 0; border-bottom: 1px solid #f1f5f9;"><strong>Alerjiler:</strong> <span style="color:#b91c1c">${d.alerjiler || '-'}</span></td></tr><tr><td style="padding: 8px 0; border-bottom: 1px solid #f1f5f9;"><strong>Sürekli İlaçlar:</strong> ${d.surekli_ilaclar || '-'}</td><td style="padding: 8px 0; border-bottom: 1px solid #f1f5f9;"><strong>Geçirilen Operasyonlar:</strong> ${d.gecirilen_operasyonlar || '-'}</td></tr></table><h3 style="background-color: #f0fdfa; color: #0f766e; padding: 10px 15px; font-size: 14px; margin-bottom: 15px; border-left: 4px solid #14b8a6; font-weight: bold;">Geçmiş Mezura ve Tartı Ölçümleri</h3><table style="width: 100%; border-collapse: collapse; margin-bottom: 30px; font-size: 12px; text-align: left;"><thead style="background-color: #f8fafc; color: #475569;"><tr><th style="border: 1px solid #e2e8f0; padding: 8px;">Tarih</th><th style="border: 1px solid #e2e8f0; padding: 8px;">Kilo</th><th style="border: 1px solid #e2e8f0; padding: 8px;">Yağ / Kas (%)</th><th style="border: 1px solid #e2e8f0; padding: 8px;">Bel / Kalça (cm)</th><th style="border: 1px solid #e2e8f0; padding: 8px;">BMI</th></tr></thead><tbody>${olcumHtml}</tbody></table><h3 style="background-color: #fef2f2; color: #b91c1c; padding: 10px 15px; font-size: 14px; margin-bottom: 15px; border-left: 4px solid #b91c1c; font-weight: bold;">Laboratuvar ve Kan Tahlili Sonuçları</h3><table style="width: 100%; border-collapse: collapse; margin-bottom: 30px; font-size: 12px; text-align: left;"><thead style="background-color: #f8fafc; color: #475569;"><tr><th style="border: 1px solid #e2e8f0; padding: 8px;">Tarih</th><th style="border: 1px solid #e2e8f0; padding: 8px;">B12 / D-Vit</th><th style="border: 1px solid #e2e8f0; padding: 8px;">Demir</th><th style="border: 1px solid #e2e8f0; padding: 8px;">Kolesterol</th><th style="border: 1px solid #e2e8f0; padding: 8px;">Açlık Şekeri</th><th style="border: 1px solid #e2e8f0; padding: 8px;">TSH</th></tr></thead><tbody>${tahlilHtml}</tbody></table><div style="margin-top: 40px; text-align: center; font-size: 11px; color: #94a3b8; border-top: 1px solid #e2e8f0; padding-top: 15px;">Bu rapor profesyonel takip amaçlıdır. DiyetTakibim Yönetim Sistemi tarafından oluşturulmuştur.</div></div>`;
...
window.randevuSelectDoldur = async function() {
    ...
    if(data) {
        s.innerHTML = '<option value="">Danışan Seçiniz...</option>' + data.map(d => `<option value="${d.id}">${d.ad} ${d.soyad}</option>`).join('');
    }
};
...
document.addEventListener('submit', async (e) => {
    ...
    if(f.id === 'form-yeni-randevu') {
        ...
        if(!hSel.value) { window.showToast('Danışan seçiniz!', 'error'); return; }
        const { error } = await supabase.from('randevular').insert([{
            hastaid: hSel.value, hastaad: hSel.options[hSel.selectedIndex].text, 
            tarih: document.getElementById('r-tarih').value, saat: document.getElementById('r-saat').value, 
            tip: document.getElementById('r-tip').value, durum: 'Bekliyor', timestamp: new Date().toISOString()
        }]);
    ...
    if(f.id === 'form-yeni-diyet') {
        const { error } = await supabase.from('diyetler').insert([{
            hastaid: window.aktifHastaId, baslik: document.getElementById('diy-baslik').value,
            sabah: document.getElementById('diy-sabah').value, ara1: document.getElementById('diy-ara1').value,
            ogle: document.getElementById('diy-ogle').value, ara2: document.getElementById('diy-ara2').value,
            aksam: document.getElementById('diy-aksam').value, ara3: document.getElementById('diy-ara3').value,
            icerik: document.getElementById('diy-notlar').value
        }]);
        if(!error) { f.reset(); window.closeModal('modal-diyet'); window.showToast('Diyet Listesi Kaydedildi!'); window.diyetleriGetir(window.aktifHastaId); }
    }

    // 3. Yeni Şablon
    if(f.id === 'form-yeni-sablon') {
        const { error } = await supabase.from('sablonlar').insert([{
            baslik: document.getElementById('s-baslik').value, sabah: document.getElementById('s-sabah').value, 
            ara1: document.getElementById('s-ara1').value, ogle: document.getElementById('s-ogle').value, 
            ara2: document.getElementById('s-ara2').value, aksam: document.getElementById('s-aksam').value, 
            ara3: document.getElementById('s-ara3').value, icerik: document.getElementById('s-notlar').value
        }]);
        if(!error) { f.reset(); window.closeModal('modal-sablon'); window.showToast('Şablon eklendi!'); window.sablonlariGetir(); }
    }

    // 4. Ölçüm
    if(f.id === 'form-yeni-olcum') {
        ...
        const { error } = await supabase.from('olcumler').insert([{
            hastaid: window.aktifHastaId, tarih: document.getElementById('o-tarih').value, kilo: kilo, vki: vki,
            yag: document.getElementById('o-yag').value || 0, kas: document.getElementById('o-kas').value || 0,
        ...
        if(!error) { f.reset(); window.closeModal('modal-olcum'); window.showToast('Ölçüm eklendi!'); window.olcumleriGetir(window.aktifHastaId); }
    }

    // 5. Tahlil
    if(f.id === 'form-yeni-tahlil') {
        const { error } = await supabase.from('tahliller').insert([{
            hastaid: window.aktifHastaId, tarih: document.getElementById('t-tarih').value, b12: document.getElementById('t-b12').value || '-',
            d_vitamini: document.getElementById('t-dvit').value || '-', demir: document.getElementById('t-demir').value || '-',
            kolesterol: document.getElementById('t-kolesterol').value || '-', aclik_sekeri: document.getElementById('t-seker').value || '-', tsh: document.getElementById('t-tsh').value || '-'
        }]);
        if(!error) { f.reset(); window.closeModal('modal-tahlil'); window.showToast('Tahlil eklendi!'); window.tahlilleriGetir(window.aktifHastaId); }
    }

    // 6. Finans ve Kasa
    if(f.id === 'form-cari-hizmet' || f.id === 'form-cari-odeme' || f.id === 'form-finans') {
        let veri = { islem_tarihi: new Date().toISOString().split('T')[0] };
        if(f.id === 'form-cari-hizmet') { veri.hastaid = window.aktifHastaId; veri.tutar = document.getElementById('ch-tutar').value; veri.tur = 'Hizmet Bedeli'; }
        if(f.id === 'form-cari-odeme') { veri.hastaid = window.aktifHastaId; veri.tutar = document.getElementById('co-tutar').value; veri.tur = 'Ödeme'; veri.odeme_yontemi = document.getElementById('co-yontem').value; }
        if(f.id === 'form-finans') { const sel = document.getElementById('f-hasta'); veri.hastaid = sel.options[sel.selectedIndex].dataset.dbid; veri.tutar = document.getElementById('f-tutar').value; veri.tur = 'Ödeme'; veri.odeme_yontemi = document.getElementById('f-tip').value; veri.islem_tarihi = document.getElementById('f-tarih').value; }
        
        const { error } = await supabase.from('cari_hareketler').insert([veri]);
        if(!error) { f.reset(); window.closeModal(f.id.replace('form-', 'modal-').replace('cari-hizmet', 'cari-hizmet').replace('cari-odeme','cari-odeme')); window.showToast('İşlem kaydedildi!'); window.cariHareketleriGetir(window.aktifHastaId); window.finanslariGetir(); }
    }
});

// ================= DİĞER VERİ GETİRİCİLER =================
window.olcumleriGetir = async function(hId) {
    const tablo = document.getElementById("tablo-olcum-gecmis"); const { data } = await supabase.from('olcumler').select('*').eq('hastaid', hId).order('tarih', { ascending: false });
...
window.tahlilleriGetir = async function(hId) {
    const tablo = document.getElementById("tablo-tahliller"); const { data } = await supabase.from('tahliller').select('*').eq('hastaid', hId).order('tarih', { ascending: false });
...
window.finanslariGetir = async function() { 
    const tablo = document.getElementById("kasa-tablosu"); const { data } = await supabase.from('cari_hareketler').select('*, danisanlar(ad, soyad)').eq('tur', 'Ödeme').order('islem_tarihi', { ascending: false }); 
    if(tablo) { tablo.innerHTML = ""; let top = 0; if(data) { data.forEach(i => { top += i.tutar; tablo.innerHTML += `<tr><td class="p-4 border-b border-gray-100">${new Date(i.islem_tarihi).toLocaleDateString('tr-TR')}</td><td class="p-4 border-b border-gray-100 font-bold">${i.danisanlar ? i.danisanlar.ad + " " + i.danisanlar.soyad : "Bilinmiyor"}</td><td class="p-4 border-b border-gray-100 font-black text-teal-700">${i.tutar} ₺</td><td class="p-4 border-b border-gray-100 text-right"><button onclick="window.cariSil('${i.id}')" class="text-red-300 hover:text-red-500"><i class="fas fa-trash"></i></button></td></tr>`; }); } if(document.getElementById("stat-kasa")) document.getElementById("stat-kasa").innerText = top + " ₺"; } 
};

// ================= SİLME İŞLEMLERİ =================
window.olcumSil = async (id) => { await supabase.from('olcumler').delete().eq('id', id); window.showToast('Silindi'); window.olcumleriGetir(window.aktifHastaId); };
window.tahlilSil = async (id) => { await supabase.from('tahliller').delete().eq('id', id); window.showToast('Silindi'); window.tahlilleriGetir(window.aktifHastaId); };
window.diyetSil = async (id) => { await supabase.from('diyetler').delete().eq('id', id); window.showToast('Silindi'); window.diyetleriGetir(window.aktifHastaId); };
window.sablonSil = async (id) => { await supabase.from('sablonlar').delete().eq('id', id); window.showToast('Silindi'); window.sablonlariGetir(); };
window.cariSil = async (id) => { await supabase.from('cari_hareketler').delete().eq('id', id); window.showToast('Silindi'); window.cariHareketleriGetir(window.aktifHastaId); window.finanslariGetir(); };

// ================= RANDEVU MENÜSÜ İŞLEMLERİ =================
window.randevuIslem = function(id, durum) { 
    let div = document.getElementById('custom-randevu-modal'); 
    if(div) div.remove(); 
    div = document.createElement('div'); div.id = "custom-randevu-modal"; div.className = "fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4"; 
    div.innerHTML = ` <div class="bg-white rounded-xl p-6 w-full max-w-sm shadow-2xl relative transition-all"> <button onclick="document.getElementById('custom-randevu-modal').remove()" class="absolute top-4 right-4 text-slate-400 hover:text-red-500 text-xl"><i class="fas fa-times"></i></button> <h3 class="text-lg font-black mb-2 text-slate-800">Randevu İşlemi</h3> <p class="text-xs font-bold text-slate-500 mb-6 uppercase tracking-wider">Şu anki durum: <span class="text-teal-600">${durum}</span></p> <div class="space-y-3"> <button onclick="window.randevuDurumGuncelle('${id}', 'Geldi')" class="w-full flex items-center justify-center gap-2 bg-emerald-50 text-emerald-700 border border-emerald-200 font-bold py-3 rounded-lg"><i class="fas fa-check-circle"></i> Danışan Geldi</button> <button onclick="window.randevuDurumGuncelle('${id}', 'İptal Etti')" class="w-full flex items-center justify-center gap-2 bg-red-50 text-red-600 border border-red-200 font-bold py-3 rounded-lg"><i class="fas fa-times-circle"></i> İptal Etti / Gelmedi</button> <div class="border-t border-gray-100 my-2 pt-2"></div> <button onclick="window.randevuKalicSil('${id}')" class="w-full flex items-center justify-center gap-2 bg-slate-800 text-white font-bold py-3 rounded-lg shadow-md"><i class="fas fa-trash"></i> Takvimden Sil</button> </div> </div> `; 
    document.body.appendChild(div); 
}
window.randevuDurumGuncelle = async function(id, yeniDurum) { document.getElementById('custom-randevu-modal').remove(); await supabase.from('randevular').update({ durum: yeniDurum }).eq('id', id); window.showToast('Randevu güncellendi!'); window.randevulariGetir(); }
window.randevuKalicSil = async function(id) { document.getElementById('custom-randevu-modal').remove(); if(confirm("Silmek istiyor musunuz?")) { await supabase.from('randevular').delete().eq('id', id); window.showToast('Randevu silindi!'); window.randevulariGetir(); } }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    danisanlariGetir(); kayitFormunuBaslat(); window.randevuSelectDoldur(); window.finanslariGetir();
