...
import { danisanlariGetir, kayitFormunuBaslat } from './modules/danisan.js?v=kadirFinal';
import { randevuFormunuBaslat } from './modules/randevu.js?v=kadirFinal';
...
// ================= PDF MOTORU (KESİLME HATASI ÇÖZÜLDÜ) =================
const pdfOlusturVeIndir = (htmlIcerik, dosyaAdi) => {
    window.showToast('PDF Hazırlanıyor, lütfen bekleyin...', 'success');
    
    // Tasarımı ekranın fersah fersah dışına 800px genişliğinde kuruyoruz ki daralıp sağdan kesilmesin!
    ...
    const opt = {
        ...
        // windowWidth: 800 motorun ekranı daraltmasını engeller
...
// ================= TAKVİM ÇEKİCİ (SADECE AYLIK İÇİN DÜZELTİLDİ) =================
                ...
                data.forEach(r => { 
                    try {
                        ...
                        window.globalCalendar.addEvent({ 
                            ...
                            start: r.tarih, // Sadece tarih veriyoruz, aylıkta tam gün görünür
                            allDay: true, 
                            color: rRenk, 
                            extendedProps: { dbId: r.id, durum: r.durum || 'Bekliyor' } 
                        }); 
                    } catch (e) {
                        console.warn("Randevu atlandı", r);
                    }
                }); 
            }
        } 
...
// ================= TÜM KAYDETME (FORM SUBMİT) MOTORLARI =================
const frmRandevu = document.getElementById('form-yeni-randevu');
if(frmRandevu) {
    let newFrm = frmRandevu.cloneNode(true);
    frmRandevu.parentNode.replaceChild(newFrm, frmRandevu);
    newFrm.onsubmit = async (e) => {
        e.preventDefault();
        ...
        const { error } = await supabase.from('randevular').insert([{
            hastaid: hastaId, hastaad: hastaAd, tarih: tarih, saat: saat, tip: tip, durum: 'Bekliyor', timestamp: new Date().toISOString()
        }]);
        ...
        if(!error) { 
            ...
            window.showToast('Randevu Hatası!', 'error');
        }
    };
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(calendarEl) {
        window.globalCalendar = new FullCalendar.Calendar(calendarEl, {
            // SADECE AYLIK GÖRÜNÜM OLARAK SABİTLENDİ (Haftalık/Günlük iptal edildi)
